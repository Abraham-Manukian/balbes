...
int main(int argc, char* argv[]) {
    ...
    SDL_SetRenderDrawColor(renderer, 0, 0, 255, 255); // Устанавливаем цвет для отрисовки (синий)
    SDL_RenderClear(renderer); // Очищаем экран (заливаем синим цветом)

    SDL_Rect rect = { 100, 100, 200, 150 }; // Определяем прямоугольник
    SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255); // Устанавливаем цвет для прямоугольника (красный)
